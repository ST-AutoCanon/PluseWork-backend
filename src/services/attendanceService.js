...
const attendanceService = {
    ...
    try {
      ...
      return {
        ...
        missed_punch_notification_enabled:
          row.missed_punch_notification_enabled ?? 0,
  ...
  upsertLoginHoursConfig: async (orgId, config) => {
    try {
      ...
      const missedPunchNotificationEnabled =
      ...
      const columnCheck = await tenantPool.execute(
        `SHOW COLUMNS FROM login_hours_config LIKE 'missed_punch_notification_enabled'`,
      );
      if (!columnCheck?.[0]?.length) {
        await tenantPool.execute(
          `ALTER TABLE login_hours_config ADD COLUMN missed_punch_notification_enabled TINYINT(1) NOT NULL DEFAULT 0 AFTER late_login_enabled`,
        );
      }
      ...
      const params = [
        ...
        missedPunchNotificationEnabled ? 1 : 0,
        lateStreakDays,
        autoMarkLate ? 1 : 0,
        escalationMode,
        actionRolesValue,
  ...
  getPunchRecordsForDate: async (employeeId, orgId, dateKey) => {
    try {
      const tenantPool = await getTenantPoolByOrgId(orgId);
      const [rows] = await tenantPool.execute(
        `SELECT employee_id, punch_status, punchin_time, punchin_device,
                punchin_location, punchout_time, punchout_device,
                punchout_location, punchmode
         FROM emp_attendence
         WHERE employee_id = ?
           AND (DATE(punchin_time) = ? OR DATE(punchout_time) = ?)
         ORDER BY GREATEST(
           COALESCE(punchin_time, '0000-00-00'),
           COALESCE(punchout_time, '0000-00-00')
         ) DESC`,
      ...
      console.error("Error in getPunchRecordsForDate:", error);
      throw error;
    }
  },
