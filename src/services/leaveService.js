...
const getLeaveTypeVariants = (value) => {
  ...
  return [...new Set(normalized.map((v) => normalizeLeaveTypeKey(v)))].filter(
    Boolean,
  );
};

const findMatchingLeaveBalanceRow = async (
  tenantPool,
  employeeId,
  leaveType,
) => {
  const targetVariants = getLeaveTypeVariants(leaveType);
  if (!targetVariants.length) return null;

  const [rows] = await tenantPool.execute(
    `SELECT id, remaining, leave_type FROM employee_leave_balances WHERE employee_id = ? ORDER BY remaining DESC, id DESC`,
    [employeeId],
  );
  ...
  const matches = rows.filter((row) => {
    if (!row || row.leave_type == null) return false;
    const rowVariants = getLeaveTypeVariants(row.leave_type);
    return rowVariants.some((v) => targetVariants.includes(v));
  });

  if (matches.length === 0) return null;

  return matches.reduce((best, row) => {
    const bestRemaining = Number(best?.remaining ?? -Infinity);
    const rowRemaining = Number(row?.remaining ?? -Infinity);
    return rowRemaining > bestRemaining ? row : best;
  }, matches[0]);
};
...
const updateLeaveRequest = async (payload, orgId) => {
  ...
  try {
    ...
    let remaining = 0;
    try {
      const row = await findMatchingLeaveBalanceRow(
        tenantPool,
        leave.employee_id,
        leave.leave_type,
      );
      remaining = row ? Number(row.remaining) || 0 : 0;
    } catch (err) {
      if (err && err.code === "ER_NO_SUCH_TABLE") {
        console.warn("employee_leave_balances missing — treating remaining=0.");
        remaining = 0;
    ...
    if (status === "Approved") {
      ...
      try {
        ...
        if (d_write > EPS) {
          try {
            const balanceRow = await findMatchingLeaveBalanceRow(
              conn,
              leave.employee_id,
              leave.leave_type,
            );
            if (balanceRow && balanceRow.leave_type) {
              const balanceRowId = balanceRow.id || null;
              const updateResult = await conn.execute(
                `UPDATE employee_leave_balances SET remaining = GREATEST(0, remaining - ?) WHERE employee_id = ? AND leave_type = ?`,
                [d_write, leave.employee_id, balanceRow.leave_type],
              );
              if (
                !updateResult ||
                !updateResult[0] ||
                Number(updateResult[0].affectedRows || 0) === 0
              ) {
                console.warn(
                  `[updateLeaveRequest] no employee_leave_balances row matched leave_type=${balanceRow.leave_type} for employee=${leave.employee_id}`,
                );
              }
            } else {
              const balanceVariants = getLeaveTypeVariants(leave.leave_type);
              if (balanceVariants.length > 0) {
                const rows = await conn.execute(
                  `SELECT id, remaining, leave_type FROM employee_leave_balances WHERE employee_id = ?`,
                  [leave.employee_id],
                );
                const candidate = (rows && rows[0] ? rows[0] : []).find((r) => {
                  const rVariants = getLeaveTypeVariants(r.leave_type);
                  return rVariants.some((v) => balanceVariants.includes(v));
                });
                if (candidate) {
                  await conn.execute(
                    `UPDATE employee_leave_balances SET remaining = GREATEST(0, remaining - ?) WHERE employee_id = ? AND leave_type = ?`,
                    [d_write, leave.employee_id, candidate.leave_type],
                  );
                } else {
                  console.warn(
                    `[updateLeaveRequest] no employee_leave_balances row matched leave_type=${leave.leave_type} for employee=${leave.employee_id}`,
                  );
                }
              }
            }
