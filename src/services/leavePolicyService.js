...
function getLeaveTypeVariants(value) {
  const raw = String(value ?? "").trim();
  if (!raw) return [];
  const base = normalizeLeaveTypeKey(raw);
  const variants = new Set([base, raw.toLowerCase()]);
  if (base) {
...
function getVariantValueByKey(map, value) {
  if (!map || !value) return undefined;
  const variants = getLeaveTypeVariants(value);
...
async function getEmployeeCarryForwards(employeeId, year, orgId) {
  ...
  try {
    ...
    (rows || []).forEach((r) => {
      ...
      for (const variant of getLeaveTypeVariants(r.leave_type)) {
        const current = Number(map[variant] || 0);
        map[variant] = Math.max(current, amount);
      }
    });
    return map;
  } catch (err) {
...
async function getEmployeePersonal(employeeId, orgId) {
  if (!employeeId || !orgId) return { gender: null, dob: null };

  try {
    const tenantPool = await getTenantPoolForOrgId(orgId);
    const [rows] = await tenantPool.execute(
      `SELECT e.dob, ep.gender
       FROM employees e
  ...
  for (const setting of settings) {
    try {
      const typeKey = String(setting.type || "")
        .trim()
        .toLowerCase();
      const used = Number(getVariantValueByKey(usedByType, typeKey) || 0);
      const annualStatic = Number(setting.value || 0);
      const policyCarry = Number(setting.carry_forward || 0);
      const empCarry = getVariantValueByKey(employeeCFMap, typeKey);
      const carryForward =
        empCarry !== undefined
          ? Math.min(Number(empCarry || 0), policyCarry)
          : 0;
...
async function buildDefaultSettingsFromLeaveTypes(orgId = null, opts = {}) {
  ...
  try {
    const [rows] = await tenantPool.execute(
      `SELECT id, type_key, display_name, gender, min_age, max_age, is_active
       FROM leave_types
...
const isGenderEligibleLeaveType = (typeKey = "", gender = "", age = null) => {
  const key = String(typeKey || "")
    .trim()
    .toLowerCase();
  const normalizedGender = normalizeGenderValue(gender);

  if (key.includes("maternity")) return normalizedGender === "female";
  if (key.includes("menstrual")) {
    if (normalizedGender !== "female") return false;
    if (age === null || Number.isNaN(age)) return true;
    return age >= 18 && age <= 52;
  }
  if (key.includes("paternity")) return normalizedGender === "male";
...
const getLeaveBalance = async (employeeId, orgId) => {
  ...
  const employeeProfile = await getEmployeePersonal(employeeId, orgId);
  const employeeGender = normalizeGenderValue(employeeProfile?.gender);
  const employeeAge = employeeProfile?.dob
    ? computeAgeYears(employeeProfile.dob)
    : null;
  ...
  if (!policies || policies.length === 0) {
    ...
    return defaults
      .filter((setting) =>
        isGenderEligibleLeaveType(setting.type, employeeGender, employeeAge),
      )
      .map((setting) => {
        const typeKey = String(setting.type || "").trim();
        return {
  ...
  return settings
    ...
    .map((setting) => {
      const typeKey = String(setting.type || "")
        .trim()
        .toLowerCase();
      const used = Number(getVariantValueByKey(usedMap, typeKey) || 0);
      const policyCarryAllowed = Number(setting.carry_forward || 0);
      const employeeCarry = getVariantValueByKey(employeeCFMap, typeKey);
      const carryForward =
        employeeCarry !== undefined
          ? Math.min(Number(employeeCarry || 0), policyCarryAllowed)
          : 0;
      const annual = Number(setting.value || 0);
      const isFixedPolicyLeave = ["maternity", "paternity"].includes(typeKey);
      const proratedAnnual = isFixedPolicyLeave
        ? annual
        : calculateProratedAllowance(
            annual,
            active.year_start,
