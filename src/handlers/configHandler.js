...
const getConfig = async (req, res) => {
  try {
    ...
    if (!orgId) return res.status(400).json({ success: false, message: "x-org-id required" });

    const data = await fetchConfig(orgId);
    console.log("Config fetched:", data); // DEBUG
...
const updateConfig = async (req, res) => {
  try {
    ...
    console.log("Saving config:", { key, value, orgId }); // DEBUG
