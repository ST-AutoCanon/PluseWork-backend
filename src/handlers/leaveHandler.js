...
const normalizeLeaveTypeToken = (value) =>
  String(value ?? "")
    .trim()
    .toLowerCase()
    .replace(/[_\-\s]+/g, " ")
...
const getAdvanceNoticeForLeaveType = async (orgId, leaveType) => {
  if (!orgId || !leaveType) return 0;

  const normalized = normalizeLeaveTypeToken(leaveType);
  if (!normalized) return 0;

  const policies = await LeavePolicyService.getAllPolicies(orgId).catch(
    () => [],
  );
  const candidates = Array.isArray(policies) ? policies : [];

  for (const policy of candidates) {
    const settings = Array.isArray(policy?.leave_settings)
      ? policy.leave_settings
      : [];
    ...
    for (const setting of settings) {
      ...
      const matched = settingValues.some((candidate) => {
        const token = normalizeLeaveTypeToken(candidate);
      ...
      if (Number.isFinite(rawNotice)) {
        return Math.max(0, Math.floor(rawNotice));
      }
    }
  }
  ...
  if (defaultMap[normalized]) return defaultMap[normalized];
...
class LeaveHandler {
  ...
  static async submitLeaveRequestHandler(req, res) {
    try {
      ...
      const advanceNoticeDays = await getAdvanceNoticeForLeaveType(
        orgId,
        leavetype,
      );
      if (advanceNoticeDays > 0) {
        const today = new Date();
        const minStart = new Date();
        minStart.setDate(today.getDate() + advanceNoticeDays);
        if (start < minStart) {
          return res
            .status(400)
            .json(
              ErrorHandler.generateErrorResponse(
                400,
                `This leave type must be applied at least ${advanceNoticeDays} day(s) in advance.`,
              ),
            );
        }
      }
