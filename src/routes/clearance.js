...
//       }
//     }
//     router.put("/update-final-lwd", clearanceHandler.updateFinalLwd);
...
// clearanceRouter.js  (or wherever your clearance routes are)
...
// POST - Add new clearance item with files
router.post("/item", upload.array('files', 5), async (req, res) => {
  try {
    const orgId = req.headers["x-org-id"];
    const createdBy = req.headers["x-employee-id"];
    ...
    const itemId = await h.addItem({   // Note: you can call service directly or through handler
      orgId,
      exitId,
      itemType,
      title,
    ...
    console.error("[CLEARANCE/POST] ERROR:", err);
    res.status(500).json({ error: err.message || "Failed to add item" });
  }
});

// PUT - Update clearance item
router.put("/item/:itemId", upload.array('files', 5), async (req, res) => {
  // ... your existing update item logic ...
});
...
// **NEW ROUTE** - Update Final LWD
router.put("/update-final-lwd", h.updateFinalLwd);
